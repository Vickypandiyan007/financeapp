...
from ..utils import new_modified, whole_year
...
from ..utils.multithreading_optimization import get_subcat_optimal_mean, get_cat_optimal_mean, get_event_subcat_optimal_mean
...
def filter_report_data(params: vessel_schemas.VesselFilterParams, db: Session = Depends(get_db), current_user: user_models.User = Depends(get_current_user)):
    ...
    filtered_df = vessel_particulars[
        (vessel_particulars['VESSEL TYPE'] == params.vessel_type) &
        (vessel_particulars['VESSEL SUBTYPE'].isin(params.vessel_subtype))]
    
    selected_vessels = merged_df[merged_df['COST_CENTER'].isin(filtered_df['COST_CENTER'].unique())].reset_index(drop=True)
    selected_vessels = selected_vessels[selected_vessels['VESSEL AGE'].between(params.vessel_age_start, params.vessel_age_end)]
    ...
    try:
        ...
        print(f"Error processing get_dd_subcat.")
